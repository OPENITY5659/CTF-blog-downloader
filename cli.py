...
from topics import filter_results, get_topic, load_topics
...
def build_parser():
    ...
    parser.add_argument("--topic", help="专题 id（配合 --list-topics 查看）")
...
def main(argv=None):
    ...
    if not args.topic and not args.keyword:
    ...
    if args.topic:
        ...
        keywords = [args.keyword] if args.keyword else topic["keywords"]
    else:
        topic_name = args.keyword
    ...
    out_dir = os.path.join(args.out, args.topic or "custom")
    os.makedirs(out_dir, exist_ok=True)

    entries = []
    failures = []
    for index, item in enumerate(results[: args.limit], start=1):
        print(f"[{index}/{min(args.limit, len(results))}] 下载：{item['title'][:60]}")
        ...
        if ok:
            ...
            md_name = os.path.basename(save_path)
            entries.append({
                "title": item["title"],
                "site": item["site"],
                "file": f"{os.path.basename(article_dir)}/{md_name}",
            })
            ...
            print(f"    失败：{detail}")
        if index < min(args.limit, len(results)) and args.delay > 0:
            time.sleep(args.delay)

    index_path = write_index(out_dir, topic_name, entries, failures)
    print(f"\n完成：成功 {len(entries)} 篇，失败 {len(failures)} 篇")
    print(f"索引：{index_path}")
    print(f"用 Obsidian / Typora 打开 {os.path.abspath(out_dir)} 即可离线检索。")
    return 0
